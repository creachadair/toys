...
// A Handshake couples a [Symmetric] with key-exchange state.
...
// error is nil. Upon successfully completing the last pattern of the handshake,
// it also returns a pair of new [Cipher] values; for all earlier patterns
// these are nil.
func (h *Handshake) WriteMessage(w io.Writer, payload []byte) (A, B *Cipher, _ error) {
	...
	for _, tok := range row {
		...
		case toys.S:
			...
			if err != nil {
				return nil, nil, fmt.Errorf("encrypt static: %w", err)
			}
		...
		case toys.EE:
			...
			if err != nil {
				return nil, nil, fmt.Errorf("compute ee DH: %w", err)
			}
		...
		case toys.ES:
			...
			if err != nil {
				return nil, nil, fmt.Errorf("compute es DH: %w", err)
			}
			h.symmetric.MixKey(key)
		...
		case toys.SE:
			...
			if err != nil {
				return nil, nil, fmt.Errorf("compute se DH: %w", err)
			}
			h.symmetric.MixKey(key)
		...
		case toys.SS:
			...
			if err != nil {
				return nil, nil, fmt.Errorf("compute ss DH: %w", err)
			}
			h.symmetric.MixKey(key)
...
// If it successfully completes the pattern, the error is nil.  Upon
// successfully completing the last pattern of the handshake, it also returns a
// pair of new [Cipher] values; for all non-final patterns these are nil.
func (h *Handshake) ReadMessage(w io.Writer, message []byte) (A, B *Cipher, _ error) {
	row := h.pattern.Pattern(h.pc)
	h.pc++
	isLast := h.pc >= h.pattern.Len()
	...
	for _, tok := range row {
		...
		case toys.E:
			if h.remoteEphemeral.IsValid() {
				...
				return nil, nil, fmt.Errorf("truncated message (%d < %d bytes)", len(message), ds)
			}
		...
		case toys.S:
			...
			if len(message) < want {
				return nil, nil, fmt.Errorf("truncated message (%d < %d bytes)", len(message), want)
			}
			dk, err := h.symmetric.DecryptAndHash(message[:want])
			if err != nil {
				return nil, nil, fmt.Errorf("decrypt static key: %w", err)
			}
